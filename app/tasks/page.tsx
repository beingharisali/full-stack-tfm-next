...
import {
  Task,
  getTasks,
  // Removed: createTask as createTaskApi,
...
export default function TasksPage() {
  ...
  const handleSaveTask = async (task: Task) => {
    setFormLoading(true);
    try {
      // Logic now only handles updating an existing task
      if (task._id) {
        ...
        // Since 'Add New Task' is removed, this block is technically dead code
        // but included for safety if a new task without _id somehow reaches here.
        // In a strictly 'no add task' scenario, you might throw an error here.
        throw new Error("Cannot save task: ID is missing.");
      }
  ...
  const handleDeleteTask = async (id: string) => {
    if (window.confirm("Are you sure you want to delete this task?")) {
      ...
      try {
        ...
        await fetchTasks();
        // Recalculate pagination logic after deletion
        if (
    ...
    <div className='min-h-screen bg-gray-50 font-sans'>
      ...
      <div className='container mx-auto p-6 md:p-8'>
        ...
        <Modal
          ...
          {editingTask && ( // Only show TaskForm if editing an existing task
            <TaskForm
              key={editingTask._id} // Use task ID as key for re-rendering
              initialTask={{
                ...editingTask,
                // Ensure all necessary fields for TaskForm are present
                  ...
                  {kanbanColumns.pending.map((task) => (
                    <div
                      ...
                      <div className='flex flex-wrap gap-2 mb-3'>
                        {task.priority && (
                          <span
                            className={`px-2 py-1 rounded text-xs font-semibold ${getPriorityTagStyle(
                              task.priority
                            )}`}>
